...
//		String paramsStr = null;
		...
		if (params != null && !params.isEmpty()) {
			...
			for (Map.Entry entry : entries) {
				...
				paramsSb.append(entry.getKey() + "=" + value + "&");
			}
			// 只有POST方法才能通过OutputStream(即form的形式)提交参数
			// if(method != HttpMethod.POST){
			url += "?" + paramsSb.substring(0, paramsSb.length() - 1);
			// }
		}
		...
		if (formParams != null && !formParams.isEmpty()) {
			...
			for (Map.Entry entry : entries) {
				String value = (entry.getValue() != null) ? (String.valueOf(entry.getValue())) : "";
				formParamsSb.append(entry.getKey() + "=" + value + "&");
			}
			formParamsStr = formParamsSb.substring(0, formParamsSb.length() - 1);
		}
		...
		try {
			...
			// logger.error("调用接口[" + url + "]失败！请求URL：" + url + "，参数：" +
			// params, e);
	...
	public static String get(String url, Map params) {
		return get(url, params, connectTimeout, readTimeout, charset);
	}
